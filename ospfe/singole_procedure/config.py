...
ACTORS_MODEL = r"""^\s*(?P<ragione_sociale>.+)\s+(?P<cf>[a-zA-Z0-9]{11,})\s*\(?(?P<ruolo>[A-Z0-9-]+)?\)?\s*$"""
TEMPI_COMPLETAMENTO_MODEL = r"""^\s*(?P<start>\d\d\d\d-\d\d-\d\d)\s*[;\-/_ ]\s*(?P<end>\d\d\d\d-\d\d-\d\d)\s*$""" 
